...
class Attack(object):
    ...
    """
    def __init__(self, attack, model_name: nn.Module, epsilon, targeted, random_start, norm, loss, device=None):
        """
    ...
    def get_logits(self,  x, *args, **kwargs):
        """
        ...
        """
        # self.model(x)
        if "begin_indicator" in kwargs:
            if isinstance(self.model, EnsembleModel):
                logits = []
                for i, model in enumerate(self.model.models):
                    for ii, module in enumerate(model):
                        if ii == 0:
                            x1 = module(x)
                        else:
                            # logits = module(x, **kwargs)
                            kwargs.update({"layer": kwargs[kwargs["model_names"][i]]})
                            logits.append(module(x1, **kwargs))

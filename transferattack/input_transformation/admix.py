...
class Admix(MIFGSM):
    ...
    def transform(self, x, **kwargs):
        ...
        """
        admix_images = torch.cat([(x + self.admix_strength * x[torch.randperm(x.size(0))].detach()) for _ in range(self.num_admix)], dim=0)
        return torch.cat([admix_images / (2 ** i) for i in range(self.num_scale)])

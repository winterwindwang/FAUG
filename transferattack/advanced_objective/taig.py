...
class TAIG(Attack):
    ...
    def __init__(self, model_name, epsilon=16/255, alpha=1.6/255, epoch=10, targeted=False, random_start=False, decay=1,
                norm='linfty', loss='crossentropy', device=None, steps=20,**kwargs):
        super().__init__('TAIG', model_name, epsilon, targeted, random_start, norm, loss, device)
        self.alpha = alpha
        self.epoch = epoch
        self.decay = decay
        self.steps = steps

    def compute_ig(self, data, delta, label_inputs, **kwargs):
        ...
        att_out = self.get_logits(scaled_inputs, **kwargs)
    ...
    def forward(self, data, label, **kwargs):
        ...
        for i in range(self.epoch):
            ...
            for x,d,y in zip(data, delta, label):
                grad.append(self.compute_ig(x,d,y, begin_indicator=begin_indicator, **kwargs))

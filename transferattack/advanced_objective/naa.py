...
class NAA(MIFGSM):
    ...
    def find_layer(self,layer_name):
        ...
        m = self.model[1]
        for layer in parser:
            if "vgg" in m.__class__.__name__.lower():
                try:
                    m = eval(f"m._modules['{layer.split('_')[0]}'][{layer.split('_')[1]}]")
                except:
                    raise KeyError(f"Error Occur on {m.__class__.__name__.lower()}")
            elif "densenet" in m.__class__.__name__.lower():
                try:
                    m = eval(f"m._modules['{layer.split('_')[0]}'].{layer.split('_')[1]}")
                except:
                    raise KeyError(f"Error Occur on {m.__class__.__name__.lower()}")
            elif "visformer" in m.__class__.__name__.lower():
                try:
                    m = eval(f"m._modules['{layer.split('_')[0]}'][{layer.split('_')[1]}]")
                except:
                    raise KeyError(f"Error Occur on {m.__class__.__name__.lower()}")
            elif "poolingvisiontransformer" in m.__class__.__name__.lower():
                try:
                    layer_block = layer.split("_")
                    m = eval(f"m._modules['transformers'][{layer_block[1]}].{layer_block[0]}[{layer_block[2]}]")
                except:
                    raise KeyError(f"Error Occur on {m.__class__.__name__.lower()}")
            elif "visiontransformer" in m.__class__.__name__.lower():
                try:
                    layer_block = layer.split("_")
                    # m._modules['blocks'][5]
                    m  =eval(f"m._modules['{layer_block[0]}'][{layer_block[1]}]")
                except:
                    raise KeyError(f"Error Occur on {m.__class__.__name__.lower()}")
            elif "swintransformer" in m.__class__.__name__.lower():
                try:
                    layer_block = layer.split("_")
                    m =eval(f"m._modules['{layer_block[0]}'][{layer_block[1]}]")
                except:
    ...
    def forward(self, data, label, **kwargs):
        ...
        for i in range(self.epoch):
            ...
            logits = self.get_logits(self.transform(data + delta), begin_indicator=begin_indicator, **kwargs)
            # Calculate the loss

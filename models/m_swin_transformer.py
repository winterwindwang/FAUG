...
class SwinTransformer(nn.Module):
    ...
    def forward_features(self, x, **kwargs):
        ...
        for i, module in enumerate(self.layers):
            if "layer" in kwargs and module.__class__.__name__ == 'SwinTransformerStage' and i == int(kwargs['layer'].split("_")[1]):
                x = module(x)
                if "begin_indicator" in kwargs and kwargs['begin_indicator']:
                    noise = feature_process(x.data, **kwargs)
                    x.data += noise
                # x = module.downsample(x)
                # for j, submodule in enumerate(module.blocks):
                #     if j == int(kwargs['layer'].split("_")[2]):
                #         if "begin_indicator" in kwargs and kwargs['begin_indicator']:
                #             noise = feature_process(x.data, **kwargs)
                #             x.data += noise
                #     x = submodule(x)

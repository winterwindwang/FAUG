...
class Visformer(nn.Module):
    ...
    def forward_features(self, x, **kwargs):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            # x = self.stage1(x)
            for i, moudle in enumerate(self.stage1):
                if "layer" in kwargs and "stage1" in kwargs['layer'] and i == int(kwargs['layer'].split("_")[1]):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            # x = self.stage2(x)
            for i, moudle in enumerate(self.stage2):
                if "layer" in kwargs and "stage2" in kwargs['layer'] and i == int(kwargs['layer'].split("_")[1]):
                    if "begin_indicator" in kwargs and kwargs['begin_indicator']:
                        noise = feature_process(x.data, **kwargs)
                        x.data += noise
                x = moudle(x)
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            # x = self.stage3(x)
            for i, moudle in enumerate(self.stage3):
                if "layer" in kwargs and "stage3" in kwargs['layer'] and i == int(kwargs['layer'].split("_")[1]):
                    if "begin_indicator" in kwargs and kwargs['begin_indicator']:
                        noise = feature_process(x.data, **kwargs)
                        x.data += noise
                x = moudle(x)

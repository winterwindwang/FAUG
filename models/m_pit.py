...
class Transformer(nn.Module):
    ...
    def forward(self, x: Tuple[torch.Tensor, torch.Tensor], **kwargs) -> Tuple[torch.Tensor, torch.Tensor]:
        x, cls_tokens = x
        ...
        for i, module in enumerate(self.blocks):
            if "layer" in kwargs and i == int(kwargs['layer'].split("_")[2]):
...
class PoolingVisionTransformer(nn.Module):
    ...
    def forward_features(self, x, **kwargs):
        ...
        for i, module in enumerate(self.transformers):
            if "layer" in kwargs and i == int(kwargs['layer'].split("_")[1]):
                x, cls_tokens = module((x, cls_tokens), **kwargs)
            else:

...
from timm.models import build_model_with_cfg, checkpoint_seq
...
class ResNet(nn.Module):
    ...
    def forward_features(self, x, **kwargs):
        x = self.conv1(x)
        if "layer" in kwargs and 'conv' in kwargs['layer']:
        ...
        if "layer" in kwargs and 'maxpool' in kwargs['layer']:
            if "begin_indicator" in kwargs and kwargs['begin_indicator']:
                noise = feature_process(x.data, **kwargs)
                x.data += noise
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            x = self.layer1(x)
            if "layer" in kwargs and 'layer1' in kwargs['layer']:
                if "begin_indicator" in kwargs and kwargs['begin_indicator']:
                    noise = feature_process(x.data, **kwargs)
                    x.data += noise
            x = self.layer2(x)
            if "layer" in kwargs and 'layer2' in kwargs['layer']:
                # if "begin_indicator" in kwargs and kwargs['begin_indicator']:
                    # x.data += torch.ones_like(x).normal_(mean=0.0, std=0.2)  # best
                noise = feature_process(x.data, **kwargs)
                x.data += noise
            x = self.layer3(x)
            if "layer" in kwargs and 'layer3' in kwargs['layer']:
                if "begin_indicator" in kwargs and kwargs['begin_indicator']:
                    noise = feature_process(x.data, **kwargs)
                    x.data += noise
            x = self.layer4(x)
            if "layer" in kwargs and 'layer4' in kwargs['layer']:
                if "begin_indicator" in kwargs and kwargs['begin_indicator']:
                    noise = feature_process(x.data, **kwargs)
                    x.data += noise
        return x
